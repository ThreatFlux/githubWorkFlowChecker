...
func TestCreateUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			update, err := manager.CreateUpdate(ctx, "test.yml", tt.action, tt.latestVersion)
			if (err != nil) != tt.wantErr {
				t.Errorf("CreateUpdate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tt.wantUpdate && update == nil {
...
jobs:
  test:
	...
	if !strings.Contains(updatedContent, "actions/checkout@v3") {
		t.Error("Update for checkout action was not applied")
	}
	if !strings.Contains(updatedContent, "actions/setup-node@v3") {
		t.Error("Update for setup-node action was not applied")
	}
}
